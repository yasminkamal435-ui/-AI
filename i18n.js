...
export const I18N = {
  ar: {
    ...
    evaluation: {
      title: 'تقييم الموديل الأساسي', sub: 'مقارنة توقعات الموديل المستخدم في المنصة بالقيمة الحقيقية على عينة تحقق جديدة، مع رسم Actual vs Predicted. ملحوظة: العينة هنا بتتولّد بصيغة تركيبية مستقلة (نفس الصيغة المستخدمة في صفحة التدريب)، فممكن دقّة R² هنا تختلف شوية عن رقم 0.9806 المعروض في الصفحة الرئيسية (اللي جاي من بيانات التدريب الأصلية).',
  ...
  en: {
    ...
    evaluation: {
      title: 'Main Model Evaluation', sub: 'Comparing the deployed model\'s predictions against ground truth on a fresh validation sample, with an Actual vs Predicted chart. Note: this sample is generated from an independent synthetic formula (the same one used on the Training page), so the R² here may differ slightly from the 0.9806 shown on the home page (which comes from the original training data).',
