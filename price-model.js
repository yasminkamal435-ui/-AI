...
export const FINISH_NAMES = { ar: ['سوبر لوكس', 'متوسط', 'على الطوب الأحمر'], en: ['Super Lux', 'Standard', 'Basic (red brick)'] };
...
export function predictPrice(spec) {
  ...
  return forward(featuresFromInputs(area, rooms, floor, age, cityIdx, finishIdx)) * 1_000_000;
}
...
export function explainPrediction(spec) {
  ...
  const withBasicFinish = predictPrice({ ...spec, finishIdx: 2 });
...
export function globalFeatureImportance() {
  ...
  const scores = {
    ...
    finish: Math.max(...[1, 2].map(f => Math.abs(predictPrice({ ...probe, finishIdx: f }) - base))),
...
/**
 * Simplified SHAP values via Monte-Carlo permutation sampling (the standard
 * approach for approximating Shapley values when there's no closed-form
 * solution): for many random feature orderings, features are added one at a
 * time from a fixed "average apartment" baseline to the actual spec's
 * values, and each feature's marginal contribution to the prediction is
 * averaged across all orderings. This is a genuine (if simplified/approximate)
 * Shapley-value estimate, not just a relabeled perturbation analysis — by
 * construction, the sum of the returned values is very close to
 * predictPrice(spec) - predictPrice(BASELINE_SPEC) (the additivity property).
 */
export const BASELINE_SPEC = { area: 120, rooms: 3, floor: 5, age: 15, cityIdx: 4, finishIdx: 1 };

export function computeShapValues(spec, nPermutations = 80) {
  const features = ['area', 'rooms', 'floor', 'age', 'cityIdx', 'finishIdx'];
  ...
  for (let p = 0; p < nPermutations; p++) {
    ...
    let current = { ...BASELINE_SPEC };
    let prevPred = predictPrice(current);
    for (const f of order) {
      current = { ...current, [f]: spec[f] };
      const newPred = predictPrice(current);
      contributions[f] += (newPred - prevPred);
      prevPred = newPred;
    }
  }
  for (const f of features) contributions[f] /= nPermutations;
