...
import { predictPrice, explainPrediction, computeShapValues, PRICE_MODEL_METRICS, CITY_NAMES, FINISH_NAMES } from './price-model.js';
...
import { trainLive, trainAndCompareAlgorithms, evaluateMainModel } from './train.js';
...
function goToPage(page) {
  ...
  if (page === 'evaluation') renderMainModelEvaluation();
}
...
function render() {
  ...
  document.getElementById('navEvaluation').textContent = D.nav.evaluation;
  ...
  document.getElementById('shapTitle').textContent = D.explain.shapTitle;
  document.getElementById('shapSub').textContent = D.explain.shapSub;
  ...
  document.getElementById('evalTitle').textContent = D.evaluation.title;
  document.getElementById('evalSub').textContent = D.evaluation.sub;
  document.getElementById('compareAlgoTitle').textContent = D.evaluation.compareTitle;
  document.getElementById('compareAlgoSub').textContent = D.evaluation.compareSub;
  if (!compareInProgress) document.getElementById('btnCompareAlgos').textContent = D.evaluation.compareBtn;
...
function renderResult() {
  ...
  const shapValues = computeShapValues(lastPrediction);
  const shapList = document.getElementById('shapList');
  shapList.innerHTML = '';
  const shapEntries = Object.entries(shapValues).sort((a, b) => Math.abs(b[1]) - Math.abs(a[1]));
  const maxShapAbs = Math.max(...shapEntries.map(([, v]) => Math.abs(v)));
  shapEntries.forEach(([key, val]) => {
    const pct = Math.max(6, Math.round((Math.abs(val) / maxShapAbs) * 100));
    const row = document.createElement('div');
    row.className = 'explain-row';
    const label = D.explain.shapNames[key] || key;
    const sign = val >= 0 ? '+' : '−';
    row.innerHTML = `<span style="min-width:140px;">${label}</span><span class="bar-wrap"><span class="bar" style="width:${pct}%; background:${val >= 0 ? 'var(--accent)' : 'var(--money)'};"></span></span><span class="amount">${sign}${fmtNum(Math.abs(val))} ${D.form.currency}</span>`;
...
async function renderMainModelEvaluation() {
  if (evalDone) return;
  evalDone = true;
  const D = d();
  const { points, metrics } = evaluateMainModel(spec => predictPrice(spec), 400);

  const metricsEl = document.getElementById('evalMetrics');
  metricsEl.innerHTML = `
    <div class="metric-card"><div class="val">${fmtNum(metrics.mae)}</div><div class="label">${D.evaluation.mae}</div></div>
    <div class="metric-card"><div class="val">${fmtNum(metrics.rmse)}</div><div class="label">${D.evaluation.rmse}</div></div>
    <div class="metric-card"><div class="val">${metrics.r2.toFixed(4)}</div><div class="label">${D.evaluation.r2}</div></div>
  `;
  ...
  const maxVal = Math.max(...points.flatMap(p => [p.actual, p.predicted]));
  evalChart = new window.Chart(ctx, {
    type: 'scatter',
    data: {
      datasets: [
        { label: lang === 'ar' ? 'توقعات الموديل' : 'Model predictions', data: scatterData, backgroundColor: 'rgba(30,132,73,0.55)', pointRadius: 3 },
        { label: lang === 'ar' ? 'خط التطابق المثالي' : 'Perfect fit', data: [{ x: 0, y: 0 }, { x: maxVal, y: maxVal }], type: 'line', borderColor: '#C97A1A', borderDash: [6, 4], pointRadius: 0, fill: false },
      ],
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      scales: {
        x: { title: { display: true, text: lang === 'ar' ? 'السعر الحقيقي' : 'Actual price' } },
        y: { title: { display: true, text: lang === 'ar' ? 'السعر المتوقّع' : 'Predicted price' } },
      },
    },
  });
}
...
const btnCompareAlgos = document.getElementById('btnCompareAlgos');
const compareAlgoStatus = document.getElementById('compareAlgoStatus');
const compareAlgoTable = document.getElementById('compareAlgoTable');

btnCompareAlgos.addEventListener('click', async () => {
  if (compareInProgress) return;
  compareInProgress = true;
  btnCompareAlgos.disabled = true;
  compareAlgoTable.innerHTML = '';
  const D = d();

  try {
    const { results } = await trainAndCompareAlgorithms({
      trainSize: 1200, valSize: 350, nnEpochs: 25,
      mainModelPredict: (x) => {
        // x is the raw 12-dim feature vector; reconstruct a spec for predictPrice
        const area = x[0] * 300, rooms = x[1] * 6, floor = x[2] * 15, age = x[3] * 40;
        let cityIdx = 0; for (let i = 0; i < 5; i++) if (x[4 + i] === 1) cityIdx = i;
        let finishIdx = 0; for (let i = 0; i < 3; i++) if (x[9 + i] === 1) finishIdx = i;
        return predictPrice({ area, rooms, floor, age, cityIdx, finishIdx });
      },
      onProgress: ({ stage }) => { compareAlgoStatus.textContent = D.evaluation.training.replace('{stage}', stage); },
    });

    compareAlgoStatus.textContent = '';
    const rows = Object.entries(results);
    const bestMae = Math.min(...rows.map(([, m]) => m.mae));
    let html = `<table class="compare-table"><thead><tr>
      <th>${D.evaluation.colAlgo}</th><th>${D.evaluation.colMae}</th><th>${D.evaluation.colRmse}</th><th>${D.evaluation.colR2}</th>
    </tr></thead><tbody>`;
    for (const [name, m] of rows) {
      html += `<tr class="${m.mae === bestMae ? 'best' : ''}"><td>${name}</td><td>${fmtNum(m.mae)}</td><td>${fmtNum(m.rmse)}</td><td>${m.r2.toFixed(4)}</td></tr>`;
    }
    html += '</tbody></table>';
    compareAlgoTable.innerHTML = html;
  } catch (e) {
    compareAlgoStatus.textContent = lang === 'ar' ? 'حصل خطأ أثناء المقارنة.' : 'An error occurred during comparison.';
  } finally {
    compareInProgress = false;
    btnCompareAlgos.disabled = false;
    btnCompareAlgos.textContent = d().evaluation.compareBtn;
  }
});
